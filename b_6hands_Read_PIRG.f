c 6hands PIRG fix L change U/t process ------------------------------------
     ...
     &,Lmax=500,limW=m*100,limW2=(Lmax*3-1)*100
c set up -----------------------------------------------------
...
c iengv=1: engv process
      ...
      dimension faiold(nn,2*m),iRGdim(Lmax),fai0(nn,2*m,Lmax)
...
c end make Urg dim. ----------------------------
      ...
      do 130
      ite0=ite0+1
      if(iRGdim(ite0).eq.0) stop
      read(5,*) iLdim
      if(iLdim.ne.iRGdim(ite0)) then
         write(6,*) 'dimension conflict',iLdim,iRGdim(ite0)
         stop
      end if
      r=ran2(-ixran)
      write(6,*) 'start_dim',iLdim,ixran
      write(80,*) 'start_dim',iLdim,ixran
c read H.F.A.result 
      do l=1,iLdim
         do inum=1,m
            do i=1,nn
...
c      end do
      ...
      if(iWF.eq.1) then
      open(unit=86,file="WF_u6_m.txt",status='unknown')
      write(86,*) iLdim
      do l=1,iLdim
         do inum=1,m
            do i=1,nn
